...
switch($_SESSION['perfil']){  
            ...
            case 'Supervisor Operativo':
                ...
                AND creado_por = '".$_SESSION['login_sesion']."' AND `estado`='activo' ORDER BY  `a`.`id_formato__trabajo_campo_entj` ASC ";
                break;
            default: 
                break;  
        } 
...
for ($i=0; $i < count($dataEncuesta); $i++) { 
    ...
    $cell24 = 'X'.$j;
    $cell25 = 'Y'.$j;
    $objPHPExcel->getActiveSheet()->setCellValue($cell1, $dataEncuesta[$i]['codigoFormatoEntj']); 
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Data Trabajo de Campo ENTJ');
...
header('Content-Disposition: attachment;filename="Data Trabajo Campo ENTJ.xls"');
