    ...
    error: function (jqXHR, textStatus, errorThrown) {
      switch (textStatus) {
        case "timeout":
          fireMessage(
            "error",
            "Se excedió el tiempo de consulta al servidor",
            1001
          );
          break;
        case "error":
...
function setValueOfName(name, value) {
  ...
  if (typeof input[0] == "undefined") input = $("select[name=" + name + "]");
  if (typeof input[0] == "undefined")
  ...
  if (typeof value == "undefined") {
    ...
    if (input.attr("type") == "checkbox" && value == "1") {
      ...
      input.each(function () {
        var meI = $(this);
        if (meI.val() == value) meI.prop("checked", "checked");
      });
    } else if (input.attr("type") == "file") {
...
function setValues(object) {
  ...
  if (typeof context != "undefined")
    for (var nameContext in context) {
      ...
      if (typeof setListaInicial != "undefined") setListaInicial();
      else alert("Existe contexto pero no setListaInicial");
    }
}
...
function initStep(registro) {
  ...
  var valores = {};
  if (
    typeof registro.estado == "undefined" ||
    registro.estado == null ||
    registro.estado == "no_iniciada"
  ) {
...
function guardarSeccion(url, last) {
  ...
  if (last === true) infoEncuesta["estado"] = "finalizada";
...
function logic(nameSource, valueForChange, arrayNamesShow, arrayNamesHide) {
  ...
  var isHidden = false;
  if (
    inputSource.attr("type") == "radio" ||
    inputSource.attr("type") == "checkbox"
  ) {
