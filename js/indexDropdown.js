...
var drop = function(info){var o = {
   ...
   open: false,
   html: {
      select: jQuery.get(info.selector)[0],
      options: jQuery.get(info.selector + ' option'),
      parent: undefined,
   },
   init: function(){
      //Setup Drop HTML
      this.html.parent = jQuery.get(info.selector)[0].parentNode
      this.html.drop = jQuery.template('<div class="drop"></div>')
      this.html.dropDisplay = jQuery.template('<div class="drop-display">Display</div>')
      this.html.dropOptions = jQuery.template('<div class="drop-options">Options</div>')
      this.html.dropScreen = jQuery.template('<div class="drop-screen"></div>')
   ...
   renderDrop: function(){ 
      var that = this;
      var parentHTML = jQuery.template('<div></div>')
      this.selected.forEach(function(select, index){ 
         var option = that.options[select.index];
         var childHTML = jQuery.template('<span class="item '+ select.state +'">'+ option.html +'</span>')
         var childCloseHTML = jQuery.template(
