		...
		DbOffline.get(idEncuesta, function(registro){
			...
			$(".item_paginador").each(function(index, element){
				...
				if(num == step){
					...
					jqElemento.attr('href', 'encuesta'+num+'.html#js_?idEncuesta='+idEncuesta);
					jqElemento.click(function(e){
						e.preventDefault();
			...
			$('#anterior').attr('href', step!=1?'encuesta'+(step-1)+'.html#js_?idEncuesta='+idEncuesta:'#').click(function(e){
				e.preventDefault();
				if(step!=1){
					guardarSeccion($(this).attr("href"));
				}
			});
			$('#siguiente').attr('href', step!=numSteps?'encuesta'+(step+1)+'.html#js_?idEncuesta='+idEncuesta:'#').click(function(e){
				e.preventDefault();
				if(step!=numSteps){
					guardarSeccion($(this).attr("href"));
				}
			...
			DbOffline.getConfig(function(config){
				...
				if(typeof initSection != 'undefined')
					initSection(registro, config);
			});
			//DbOffline.printDB();
		});
	});
