...
var indexUpdate = null;
    ...
    console.debug(regsContext);
		...
		for(var indice in regsContext[nameContext]['regs']){
			...
			if(mayorIndice < parseInt(regsContext[nameContext]['regs'][indice]['indice'])){
    ...
    var dialog, form, indice = 0,
      ...
      lgbtiq = $( "#lgbtiq" ),
      relacion = $( "#relacion" ),
      allFields = $( [] ).add( nombre ).add( edad ).add( genero ).add( discapacidad ).add( desplazado ).add( lgbtiq ).add( relacion ),
      tips = $( ".validateTips" );
    ...
    function getButtonOptions(indice){
      return "<td><button class='btn btn-danger btn-xs' title='Eliminar' onclick=\"eliminar('"+indice+"')\" ><i class='fa fa-times'></i></button> <button class='btn btn-info btn-xs' title='Editar' onclick=\"editar('"+indice+"')\" ><i class='fa fa-pencil-square-o'></i></button></td>";
    }
...
function setValoresDeFormContext(object){
  for(var index in context[nameContext]){
    ...
    if(typeof object[context[nameContext][index]] == "undefined" ){
      ...
      if(input.attr('type') == 'checkbox' && object[context[nameContext][index]] == '1'){
...
function obtenerValoresDeFormContext(){
	...
	for(var index in context[nameContext]){
		...
		var input = $('input:radio[name='+context[nameContext][index]+']:checked');
		if(typeof input[0] != 'undefined'){
		...
		input = $("select[name="+context[nameContext][index]+"]");
		if(typeof input[0] != 'undefined'){
			objeto[context[nameContext][index]] = input.val()==""?null:input.val();
			continue;
		}
    ...
    function openDialog(){
      dialog.dialog( "open" );
      if(typeof afterOpenDialog != 'undefined')
        afterOpenDialog();
    }
    function closeDialog(){
      dialog.dialog( "close" );
      if(typeof afterCloseDialog != 'undefined')
        afterCloseDialog();
    }
